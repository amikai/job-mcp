...
**Goal:** Add careers-page discovery, site-bound provider operations, and an
aligned multi-company debug CLI without changing `companies.yaml`.
...
- [x] Implement `cmd/oracle` with companies, search, facets, and detail
  commands, using `--company` for curated names or direct careers URLs.
