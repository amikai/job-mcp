...
// The fixture has no remote postings, so remote detection uses minimal markup.
...
// wantDetail omits the large Description, which is checked with assert.Contains.
...
// The fixture uses Easy Apply, so this test supplies the external-ATS markup;
// the parser only accepts url as the first query parameter.
